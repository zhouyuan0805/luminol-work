...
sample_series = test_series.resample('1T', how='mean').bfill()

compare_same = sample_series / sample_series.shift(1, freq='1D')
print compare_same.dropna()
print compare_same
compare_same.to_csv(read_path + 'compare_previous_1D_result.csv', header=False)

my_detector = AnomalyDetector(read_path + 'compare_previous_1D_result.csv')
...
axes[1].legend(['day-on-day compare series'], loc='best')
axes[1].set_title('compare current data to previous day data result series')
axes[2].legend(['score series'])
axes[2].set_title('luminol algorithm detect day-on-day series result: score')
...
plt.suptitle('Anomaly detect result figure: 1-day-on-day')
plt.savefig(save_figure + 'resample_1DoD_LuminolScore.png', dpi=300)
