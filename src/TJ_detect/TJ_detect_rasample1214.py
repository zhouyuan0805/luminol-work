...
for i in range(7, len(metrics)):
    ...
    for j in range(len(group_index)):
        ...
        series_to_csv(resample_path + data_flag + '_resample_1min.csv', detect_min_series)

        detector = AnomalyDetector(resample_path + data_flag + '_resample_1min.csv')
        score = detector.get_all_scores()
        print score
        score_df = score_to_df(score)
        score_df.to_csv(resample_score_path + str(data_flag) + '_resample_1min_score.csv', index=False, header=False)
        print score_df.head()
        ...
        axes[0].legend(['resample 1min series'])
        axes[1].plot(score_series, color='r', linestyle='-')
        axes[1].legend(['score series'])
        plt.savefig(resample_figure_path + str(data_flag) + '_resample_1min.png', dpi=300)
        # plt.show()
        plt.close()
